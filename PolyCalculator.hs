...
In this task, you will create a REPL (Read-Evaluate-Print Loop) which will read
polynomials from the command line and perform various operations on them.

Your program should handle the following commands and reply with the
...
<whitespaces> denotes one or more consecutive whitespace characters (' ' aka
ASCII 0x20 aka the longest key on your keyboard)
...
1. CMD load<whitespaces><polynomial>
   ...
   REP If the polynomial can not be parsed successfully, print "Could not parse
   the polynomial."
...
2. CMD evaluate<whitespaces><number>
   ...
   REP If there is no current polynomial, print "No polynomial loaded."
       Otherwise evaluate the current polynomial at the given point and print
       one number - its value at that point.

3. CMD memorise
   ACT Save the current polynomial to memory.
   REP If there is no current polynomial, print "No polynomial loaded."
       Otherwise save the current polynomial to memory and print "Polynomial
       saved."

4. CMD recall
   ACT retrieve the saved polynomial from memory, replacing the current
      polynomial with it.
...
5. CMD clear
   ACT Clear memory, removing the polynomial stored there (but not the current
      polynomial!).
...
7. CMD expand
   ACT expands the current polynomial using the expand function from A2 and
      makes it current
   REP If there is no current polynomial, print "No polynomial loaded."
       Otherwise print the expanded polynomial using its Show instance.

8. CMD simplify
   ACT Simplifies the current polynomial using the simplify function from A2
...
If any other command is entered, including if a correct command has unexpected
parameters, print "Unbearable action!"

Your implementation should ignore any extra spaces after commands.

All messages you'll need to print are provided for you in constants. Please,
use them.
...
DO NOT print these marks in your implementation! They are only given in
